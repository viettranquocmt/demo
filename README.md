## 1. Decision Variables (Variable Mapping)
...
For a computational SAT solver, every unique $(i, j)$ pair is mapped to a unique positive integer (e.g., $1, 2, 3$). A negative sign denotes logical negation ($\neg$).

## 2. The Availability Constraint (Busy Rule)
...
* **CNF Translation:** Because the "busy" schedule is known statically from the dataset, whenever $Busy(i, j)$ is true, we strictly enforce that $x_{i,j}$ must be false. This forms a single-literal clause (a unit clause):
$$(\neg x_{i,j})$$

## 3. The No Double-Booking Constraint (Overlap Rule)
...
* **CNF Translation:** Using logical equivalence (implication elimination and De Morgan's laws), this rule dictates that invigilator $i$ cannot be assigned to both shift $j$ and shift $k$ simultaneously. For every pair of overlapping shifts $j$ and $k$, and for every invigilator $i$, we generate a clause containing two negative literals:
$$(\neg x_{i,j} \vee \neg x_{i,k})$$

## 4. The Exact Capacity Constraint
